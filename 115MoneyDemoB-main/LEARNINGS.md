...
## 15. 兩頁的設計 token 命名不同，跨頁複製 CSS 會**安靜地**掉樣式

**問題**：把 `index.html` 寫好的頁尾 CSS 原封不動貼到 `database.html`，版面結構會對，但顏色全掉——文字變成繼承來的預設色，看起來像「沒套到樣式」而不是「壞掉」，掃一眼很容易放過。

**原因**：兩頁的設計 token 是各自演化的兩套命名——`index.html` 用 `--ui-*`（`--ui-muted`、`--ui-ink-inverse`），`database.html` 用 `--np-*`（`--np-muted-ondark`、`--np-text-ondark`）。**底層色值完全一樣**（`#A79C8C` / `#EDE6D3`），只有名字不同。而 CSS 自訂屬性找不到宣告時不會報錯、不會 fallback 到任何東西，`color: var(--ui-muted)` 在 `database.html` 就等於整條宣告作廢，屬性回到繼承值。DevTools 也只會把它畫成一般的無效宣告，不會像語法錯誤那樣明顯。

**解法**：跨頁搬 CSS 時，先把要搬的區塊裡所有 `var(--…)` 抓出來，逐一確認目標頁面有沒有同名宣告；沒有的要**對映**到目標頁的等價 token，不是複製。這次的對映表寫進了 `HANDOFF-v2.md`。反過來說，寫死的字面值（本例的底色 `#1E1410`、`rgba(237,230,211,…)`）跨頁搬是安全的——所以「哪些該用 token、哪些該寫死」在多頁專案裡不只是風格問題。另外別假設「同一份 CSS 在兩頁都需要」：`index.html` 有一條全域 `h2::after` 朱紅裝飾線會漏到頁尾標題上、需要 `display:none` 覆蓋，`database.html` 沒有那條規則，**刻意不加**那個覆蓋才是對的。

## 16. 「顯示的日期」與「結構化資料的日期」是兩件事，不要一起砍

**問題**：使用者說「pc 跟 mobile 都不用製作日期」。直覺是把所有「製作日期」相關的東西都移除，包含 JSON-LD 的 `datePublished`。

**原因**：這兩者服務的對象不同。畫面上的日期是給讀者看的版面元素；JSON-LD 的 `datePublished`/`dateModified` 是給搜尋引擎的**不顯示**中繼資料，而 Google 對 `NewsArticle` 這個 type 是**要求**要有 `datePublished` 的。把它一起砍掉，對「畫面不要顯示」這個需求沒有任何幫助，卻會讓搜尋結果的日期資訊變差——是純損失。

**解法**：拆開處理——畫面上的 `<span>` 整段刪掉（連同只為它存在的 `.footer-date-sep` 規則，別留死 CSS），JSON-LD 保留，並**在該處寫註解說明這是刻意的**，否則下一個人（或下一個 agent）會把它當成漏改而「修」掉。同時把這個判斷主動告訴使用者，讓他有機會否決——這種「使用者沒明說、但我替他做了取捨」的地方一定要講，不能默默決定。

順帶一個維護陷阱：改「更新日期」時每頁要改**三處**（`<time>` 的 `datetime` 屬性、`<time>` 的畫面文字、JSON-LD 的 `dateModified`），漏掉任何一處都不會有錯誤訊息、畫面也正常，只有機器讀到的日期會不一致。已在 `<footer>` 上方留註解列出這三處。

## 17. 兩字職稱要對齊四字職稱：`text-align-last` 必須配 `text-align: justify`

**問題**：`製作團隊` 名單裡職稱長度不一（`監製`、`攝影` 兩字 vs `製作統籌`、`網頁工程` 四字），要讓兩字的左右撐開對齊四字的欄寬，直覺是補全形空白（`監　　製`）——但那會污染文字內容，複製貼上、螢幕閱讀器、日後改名單全都會受影響。

**原因**：`text-align-last` 控制的是「最後一行」的對齊方式，而單行文字**同時是第一行也是最後一行**。但瀏覽器只有在該元素本身處於 justify 情境時才會去兩端對齊——只寫 `text-align-last: justify` 而沒有 `text-align: justify`，多數瀏覽器不會生效。

**解法**：兩條一起給。搭配 grid 的 `grid-template-columns: max-content minmax(0, max-content)`，欄寬自動取最長的職稱，短的靠 `text-align-last` 均分撐開，兩字與四字的右緣就對齊了，**文字內容維持乾淨的 `監製`**。這次五個職稱都解析成 56px。另外右欄用 `minmax(0, max-content)` 而不是 `1fr`：用 `1fr` 名字欄會被撐到容器右緣，整組看起來偏左；`max-content` 讓兩欄貼齊內容再整組置中，職稱與名字才是一個對齊的整體。

## 18. flex 的換行看的是**容器**寬度，不是視窗寬度——所以「手機要一行」不能靠改視窗來測

**問題**：需求是「`.footer-dates` 在手機要一行」。想驗證就會想去把視窗縮到 375px 看看。這個環境的 `resize_window` 改不動 layout viewport（見第 14 條），於是看起來像「沒辦法驗證」。

**原因**：其實根本不需要改視窗。`flex-wrap: wrap` 的換行判斷只跟**該 flex 容器當下的可用寬度**有關，跟視窗多寬沒有直接關係——視窗寬度只是透過層層 padding 間接決定容器寬度而已。

**解法**：兩步，都不需要真的縮視窗——
1. **量文字實寬**：插一個隱藏探針，套上同樣的 `font-size`/`font-family`，量出整段內容需要多少 px（當時：12px 下需 260px），再減掉各層 padding 算出目標視窗下的可用寬度（320px 視窗剩 280px）→ 得到「放得下」的結論，而且是算出來的、不是看出來的。
2. **測換行行為**：直接把容器 `width` 暫時綁死成目標值，看它有沒有換行，測完還原。

同樣的推理適用於任何 flex/grid 的 RWD 行為驗證。真的非得看 layout viewport 的（例如 `@media` 查詢本身是否命中），才需要真機或真的能改 viewport 的工具。

## 19. 中文網頁排版有兩條方向相反的間距規則，一起處理才不會顧此失彼

**問題**：為了讓 HTML 原始碼的行不要太長，在中文句子中間換行——**漢字與漢字之間的換行會被瀏覽器渲染成一個可見的空格**。實測一頁就有 39 處，像「卡片是牛皮紙色，␣強調色取自公文的朱印紅」。這種洞在中文頁面上很常見而且很難用肉眼掃出來，因為每一處只差一個字寬。

**原因**：HTML 的空白折疊規則（連續空白＋換行 → 一個空格）是為西文設計的，西文本來就用空格分詞，所以換行變空格沒有副作用。中文沒有詞間空格，同樣的規則就變成在句子中間插入不該有的空隙。

**解法**：把漢字之間的換行接起來（`([CJK])\n\s+([CJK])` → `\1\2`），代價是原始碼會有長行，這是中文 HTML 的正常樣貌，不要為了行寬再折回去。

**但修的時候極容易矯枉過正**——中日文排版其實是兩條方向相反的規則：

| 情境 | 規則 |
|---|---|
| 漢字 ↔ 漢字 | **不留**空隙 |
| 漢字 ↔ 拉丁字母／數字 | **要留**一個空隙（可讀性，也是 W3C《中文排版需求》的建議） |
| 拉丁字母／`</code>` ↔ 全形標點（。，、；：） | **不留**空隙——標點本身已有留白，補了就是雙重間距 |

這次第二輪把標籤邊界（`…的\n<em>政黨色編碼</em>`）一起接掉時，連 `視窗 <strong>713px</strong>`、`需要時用 <code>::after</code>` 這種**該留**的空隙也吃掉了，畫面變成 `視窗713px`。最後要再補 50 處回去。所以修這個問題時，兩條規則要一起寫、一起驗，不能只跑其中一邊。驗證方式是分別數兩種違規數量，而不是只數一種。

## 20. 「說明設計系統的頁面」最容易犯的錯，就是自己不守那套系統

**問題**：做了一頁 `design-system.html` 說明專案的 5 級字級與 token 規範。回頭用量測檢視，發現這一頁自己用了 **13 個不同的 font-size**，其中 **7 個擠在 9.8px–12.5px**（0.66 / 0.68 / 0.72 / 0.78 / 0.86rem，各處隨手寫的）；而且 h2 = 32px、h3 = 17px、內文 = 16.4px，**h3 只比內文大 4%**，階層完全靠顏色在撐。另外還有 4 處 inline `style="margin-…"`，成因是同一個 class 被用在兩個不同階層，只好逐處補丁。

**原因**：寫文件頁時的心態是「先把內容講清楚」，每個區塊需要一個「稍微小一點的字」時就順手寫一個 rem 值，這些決定分散在幾百行 CSS 裡，單獨看每一個都合理。系統性的問題只有把整頁的值**一次收集起來排序**才看得見。

**解法**：驗收自己做的東西時，用量的、不要用看的。三個很便宜又極有效的指標——
1. **整頁有幾個不同的 font-size？** 超過 6–7 級就是失控。收斂方式不是硬塞回既有 token（文件頁確實需要專案沒有的角色），而是**只補最少數量的新 token 並寫明理由**，然後把所有一次性數值指過去。目標是 `font-size: 0.__rem` 在檔案裡歸零。
2. **相鄰階層的比值是多少？** h3/內文 = 1.04 代表那一層沒有在做事。拉到 1.25 以上才讀得出階層。
3. **有幾個 inline style？** 每一個都在說「有個 class 被用在它不該用的地方」。這次拆成 `.section-lede`／`.sub-lede` 之後，4 個 inline style 全部消失。

順帶一條：**不要用「縮小」來表示「這是程式碼」**。`code { font-size: 0.85em }` 疊在 `--type-caption`（12.2px）上只剩 10.4px，而表格裡的 token 名稱正是讀者最需要看清的內容——最重要的東西變成全頁最小的字。改成 `0.94em`，靠等寬字體與顏色區分就夠了。
